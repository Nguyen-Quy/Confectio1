...
def index(request):
    ...
    category = product_models.Category.objects.all()
    product = product_models.Product.objects.all()
    lst_product = product[:3]

    return render(request, "main/index.html", {'lst_product': lst_product, 'category': category, 'today': now})
...
def contact(request):
    ...
    if request.method == 'POST':
        result = "Form có post"
        form = forms.FormContact(request.POST, core_models.Contact)
    ...
    return render(request, "main/contact.html", {'today': now, 'form': form, 'result': result, 'success': success, 'last_visit': last_visit})
